...
async function check() {
  const { data: assignments } = await supabase.from("schedule_assignments").select("id, event_rule_id, event_date").limit(5);
  console.log("Assignments for rule:", assignments);
  const { data: notifs } = await supabase.from("whatsapp_scheduled_notifications").select("*").limit(1);
