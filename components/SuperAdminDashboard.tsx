  ...
  const logsByOrg = useMemo(() => {
    ...
    usageLogs.forEach((log: any) => {
      const orgData = Array.isArray(log.organizations)
        ? log.organizations[0]
        : log.organizations;
      const minData = Array.isArray(log.organization_ministries)
        ? log.organization_ministries[0]
        : log.organization_ministries;
                          ...
                          <td className="px-6 py-4 text-center">
                            <button
                              onClick={async () => {
                                if (
                                  window.confirm(
                                    `Tem certeza que deseja remover permanentemente o usuário ${u.name}?`,
                                  )
                                ) {
